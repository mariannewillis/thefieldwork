...
/**
 * Taking a course down for good.
 *
 * `DeleteWorkshop`'s sibling, and the same shape: subordinate until it is
 * asked for, and it never deletes on the first press. The confirm step is a
 * second control that says what will go, not a browser dialog — a `confirm()`
 * box is chrome she cannot read her own course's name in.
 *
 * Nothing refuses this the way a paid-up workshop refuses it. Courses are not
 * on sale, so there is no record of anybody's money to take down with the run.
 * When they are, this needs the guard `deleteWorkshop` has.
 */
...
export default function DeleteCourse({
  ...
  const [state, formAction, pending] = useActionState(deleteCourse, INITIAL);
      ...
      {!asked ? (
        <div className="mt-3 flex flex-wrap items-start justify-between gap-x-8 gap-y-4">
          <p className="max-w-[54ch] text-[17px] leading-relaxed text-ink-soft">
            Deleting removes this course from the site and takes the whole run
            of dates with it. Untick &ldquo;Show this on the site&rdquo; above
            instead and it comes down but is kept.
          </p>
          <button
            type="button"
        ...
        <form action={formAction} className="mt-3">
          ...
          <p className="mt-2 max-w-[54ch] text-[17px] leading-relaxed text-ink-soft">
            There is no undo. Every date in the run, everything written about
            them, and every picture chosen for the course goes with it.
          </p>
