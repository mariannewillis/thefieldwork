 ...
 *
 * FOUR THINGS SHE CAN DO HERE and no more: add a picture, add a document, paste
 * a film's address, and take something out. There is no rename, no folders, no
 * tags and no editing of pictures — the library is a place to find things, and
 * every one of those would be a second job bolted onto it.
 *
 * IT WAS FIVE. `describeAsset` let her write a name and a description onto the
 * library row itself, and it has gone, along with the `alt` column it wrote to.
 * The same photograph does not mean the same thing in two places: it is the
 * garden room on a workshop and the room she works in on a session, and
 * `WorkshopImage.alt`, `CourseImage.alt` and `ServiceImage.alt` are REQUIRED
 * columns on the pages that use it. So the library was asking her for a sentence
 * she had to write again anyway, and then keeping an answer nothing read.
 *
...
/** What an action that either worked or refused hands back. */
